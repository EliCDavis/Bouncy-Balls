...
public class DebugGUI : MonoBehaviour
{
    ...
    [SerializeField] public bool drawInBuild = true;

    [SerializeField] public bool displayGraphs = true;
    ...
    void DrawGraphs()
    {
        ...
        // {
        //     GUI.Box(new Rect(
        //         Screen.width - graphBlockWidth - graphLabelWidth - 5 - shiftLeft,
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            ...
            {
                // GUI.Box(new Rect(Screen.width - graphWidth - shiftLeft, 0 + graphBlockHeight * graph.group, graphWidth, graphHeight), "", boxStyle);
            }

            graph.Draw(new Rect(Screen.width - graphWidth - shiftLeft, 0 + graphBlockHeight * graph.group, graphWidth, graphHeight));
        }
        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect(Screen.width - graphWidth - minMaxXOffset - shiftLeft, groupOrigin, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(new Rect(Screen.width - graphWidth - minMaxXOffset - shiftLeft, groupOrigin + graphHeight - textSize.y, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    ...
                    GUI.skin.font = font;
                    GUI.Label(new Rect(Screen.width - graphWidth - xOffset - shiftLeft, yOffset, xOffset, graphHeight), labelGuiContent);
        ...
        foreach (var group in graphGroupBoxesDrawn)
        {
            ...
            if (
                mousePos.x < Screen.width && mousePos.x > Screen.width - graphWidth - shiftLeft &&
            ...
            )
            {
                ...
                foreach (GraphContainer graph in graphDictionary.Values)
                {
                    if (graph.group == group)
                    {
                        minMaxTextStyle.normal.textColor = graph.color;
                        GUI.color = Color.white;
                        labelGuiContent.text = graph.GetValue(graphMousePos).ToString("0.00");
                        GUI.Label(new Rect(mousePos.x - 9, group * graphBlockHeight - 13, 45, 50), labelGuiContent, minMaxTextStyle);
                        yOffset += 8f;
                    }
                }
                break;
